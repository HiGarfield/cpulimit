 ...
 *
 * @note The conversion on line 452 uses implementation-defined behavior when
 *       the value cannot be represented in pid_t (C89 section 3.2.1.2).
 *       However, the subsequent round-trip check correctly detects overflow
 *       on all common platforms (Linux, macOS, FreeBSD) where pid_t is a
 *       signed integer type. This approach is preferred over no overflow
 *       checking, as there is no portable way to check pid_t limits at
 *       compile time in C89/POSIX.1-2001.
 */
