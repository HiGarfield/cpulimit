...
pid_t getppid_of(pid_t pid) {
    ...
    pid_t ppid;
    /*
     * struct proc_taskallinfo exceeds 256 bytes; allocate on heap to stay
     * within the per-function stack-object size limit.
     */
    task_info = (struct proc_taskallinfo *)malloc(sizeof(*task_info));
    if (task_info == NULL) {
        return (pid_t)(-1);
    }
