...
pid_t getppid_of(pid_t pid) {
     ...
     * Allocate once and reuse to avoid repeated malloc/free overhead in
     * ancestry walks (is_child_of() calls this function in a loop).
     */
    static struct proc_taskallinfo *task_info = NULL;
    if (task_info == NULL) {
        task_info =
    ...
    return (get_proc_taskinfo(pid, task_info) == 0)
...
static int read_process_info(pid_t pid, struct process *proc, int read_cmd) {
    /*
     * Heap-allocate: struct >= 128 bytes must not live on the stack.
     * Allocate once and reuse across calls to avoid per-PID malloc/free
     * overhead during full-process-list iteration.
     */
    static struct proc_taskallinfo *task_info = NULL;
    ...
    return proc_taskinfo_to_proc(task_info, proc, read_cmd);
}
