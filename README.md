...
- Limit PID 1234 to 25% of one CPU core:

  ```sh
  cpulimit -l 25 -p 1234
  ```

- Limit processes named `myapp` to 30% of one CPU core:

  ```sh
  cpulimit -l 30 -e myapp
  ```

- Run `myapp --option` and limit it to 50% of one CPU core:

  ```sh
  cpulimit -l 50 -- myapp --option
  ```

- Limit `myapp` and its child processes to 40% of one CPU core:

  ```sh
  cpulimit -l 40 -i -e myapp
  ```
